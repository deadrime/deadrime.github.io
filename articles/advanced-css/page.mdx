...
```
    ...
    для выбранных `<input type="radio">`, `<input type="checkbox">` или `<option>` внутри `<select>`
    для кнопок
    когда у элемент пуст (нет children), можно применить `.element:empty { display: none }` вместо условного рендеринга 
    первый ребенок своего родителя
    первый ребенок такого типа, например первый `div`
    последний ребенок своего родителя
    последний ребенок такого типа, например последний `div`
    ...
    , `:optional`, `:required` - для валидации форм
...
их нельзя создать, выбрать(querySelector-ом) и менять при помощи JS.
  ...
  добавить какой-то контент после элемента (создать псевдоэлемент, который является последним потомком)
    до (создать псевдоэлемент, который будет первым потомком)
    стилизовать первую букву
