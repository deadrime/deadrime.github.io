...
const Logo = () => {
  return (
    <Link href={'/'} className="font-fira text-md">
      {'<'}Zhenya{'>'}
    </Link>
  );
};
    ...
    <Providers>
      ...
      >
        ...
        <nav className="mx-auto gap-8 hidden sm:flex text-body2 font-bold [&>a]:focus-visible:underline underline-offset-6 [&>a]:focus-visible:outline-8 [&>a]:focus-visible:ring-0 [&>a]:focus-visible:rounded-none">
          <Link href="/#technologies" className="text-secondary">
            С чем работал
          </Link>
          <Link href="/#my-experience" className="text-secondary">
            Опыт работы
          </Link>
          <Link href="/blog" className="text-secondary">
            Блог
          </Link>
          <Link href="/snippets" className="text-secondary">
            Сниппеты
          </Link>
        </nav>
      ...
      >
        ...
        <span className="text-neutral text-body2">
          © Трофимов Евгений, 2024
        </span>
      </footer>
    </Providers>
  );
